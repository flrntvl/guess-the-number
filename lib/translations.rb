...
module Translation
  TRANSLATIONS = {
    ...
    fr: {
      ...
      play_again: 'Rejouer ? (oui/non)',
      yes_answers: %w[oui o],
      yes_no_valid: %w[oui o non n]
    }
  }.freeze
  ...
  class << self
    ...
    def select_language
      @locale = loop do
        puts TRANSLATIONS[:en][:select_language]
        input = gets.chomp.downcase.to_sym
        break input if TRANSLATIONS.key?(input)

        puts "Invalid: #{TRANSLATIONS.keys.join(', ')}"
      end
    end
